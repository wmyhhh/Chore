...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            original = place.ant
            if original.is_container and original.can_contain(self):
                original.ant_contained = self
            elif not original.is_container and self.can_contain(original):
                self.ant_contained = original
                place.ant = self
            else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        step = 0
        while not check_place.is_hive:
            if self.lower_bound <= step and step <= self.upper_bound:
                if check_place.bees != []:
                    return random_bee(check_place.bees)
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.cooldown != 0:
            ...
            if self.place.bees != []:
                selected = random_bee(bees_list)
                selected.reduce_health(selected.health)
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        "*** YOUR CODE HERE ***"
        super().action(gamestate)
        enforced = self.place.exit
        while enforced is not None:
            if enforced.ant is not None:
                enforced.ant.double()
            enforced = enforced.exit
        # END Problem 12
