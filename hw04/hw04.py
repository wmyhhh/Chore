...
def balanced(m):
    ...
    pl = end(l)
    pr = end(r)
    if is_planet(pl) and is_planet(pr):
        if length(l) * mass(pl) != length(r) * mass(pr):
            return False
    else:
        if length(l) * total_mass(pl) != length(r) * total_mass(pr):
            return False
        else:
            return balanced(pl) and balanced(pr)
    return True
''' 
GPT suggest:
def balanced(m):
    ...
    # 判断左右力矩是否相等
    torque_l = length(l) * total_mass(pl)
    torque_r = length(r) * total_mass(pr)
    torque_balanced = torque_l == torque_r

    # 递归判断左右结构是否也平衡
    return torque_balanced and balanced(pl) and balanced(pr)
    '''
